...
* C: 支持C(versions ≥ C89)，通用的编程语言，底层和高效，广泛应用于系统级开发和嵌入式设备。并且镜像包括Linux必须的C、Cpp工具链
* CPP：支持CPP（11,14,17），多范式的编程语言，是C语言的扩展，具备面向对象和泛型编程能力，广泛应用于系统级开发和大规模软件项目。并且镜像包括Linux必须的C、Cpp工具链
...
* Pyspark: 提供基于Python的Spark编程接口，用于大规模数据处理和分析，提供了强大的并行计算能力和丰富的数据操作函数，适合在分布式环境中进行高效的数据处理和机器学习任务。在环境中提供sqlite数据库
...
* SQL: https://github.com/jupyter-xeus/xeus-sqlite
